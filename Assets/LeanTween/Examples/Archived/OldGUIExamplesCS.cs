...
public class OldGUIExamplesCS : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        grumpyRect = new LTRect(0.5f * w - grumpy.width * 0.5f, 0.5f * h - grumpy.height * 0.5f, grumpy.width,
            grumpy.height);
        beautyTileRect = new LTRect(0.0f, 0.0f, 1.0f, 1.0f);

        LeanTween.move(buttonRect2, new Vector2(0.55f * w, buttonRect2.rect.y), 0.7f)
