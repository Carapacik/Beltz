...
public class PathSplinePerformance : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        var nodeLength =
            trackNodes +
            1; // We need to add some extra because the first and last nodes just act as *guides* to the first and last curvature
        for (var i = 0; i < nodeLength; i++)
        {
            var x = Mathf.Cos(degree * Mathf.Deg2Rad) * circleLength + Random.Range(0f, randomRange);
            var z = Mathf.Sin(degree * Mathf.Deg2Rad) * circleLength + Random.Range(0f, randomRange);
        ...
        randList[0] =
            randList
                [randList.Count - 1]; // set the zero-ith one as the last position so it will flow smoothly into the first curve
    ...
    private void Update()
    {
        ...
        if (trackPosition > 1f)
            trackPosition =
                0f; // We need to keep the ratio between 0-1 so after one we will loop back to the beginning of the track
    }
