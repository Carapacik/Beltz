...
public class GeneralBasic : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LeanTween.moveX(avatarScale, avatarScale.transform.position.x + 5f, 5f)
            .setEase(LeanTweenType.easeOutBounce); // Simultaneously target many different tweens on the same object 

        // Move Example
        LeanTween.move(avatarMove, avatarMove.transform.position + new Vector3(-9f, 0f, 1f), 2f)
        ...
        LeanTween.delayedCall(gameObject, 14f, () =>
            {
                for (var i = 0; i < 10; i++)
                {
                    ...
                    // Instantiate Avatar
                    var dude = Instantiate(prefabAvatar, Vector3.zero, prefabAvatar.transform.rotation);
                    ...
                    // Push into the wheel
                    LeanTween.moveZ(dude, 0f, 0.3f).setDelay(1.2f + i * 0.4f).setEase(LeanTweenType.easeSpring)
                        .setOnComplete(
                            () => { LeanTween.rotateAround(rotator, Vector3.forward, -1080f, 12f); }
                        );

                    // Jump Up and back down
                    LeanTween.moveLocalY(dude, 4f, 1.2f).setDelay(5f + i * 0.2f).setLoopPingPong(1)
