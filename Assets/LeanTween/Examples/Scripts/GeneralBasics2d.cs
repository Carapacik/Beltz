...
public class GeneralBasics2d : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LeanTween.moveX(avatarScale, avatarScale.transform.position.x + 1f, 5f)
            .setEase(LeanTweenType.easeOutBounce); // Simultaneously target many different tweens on the same object 

        // Move Example
        LeanTween.move(avatarMove, avatarMove.transform.position + new Vector3(1.7f, 0f, 0f), 2f)
        ...
        LeanTween.scale(avatarScale, new Vector3(0.2f, 0.2f, 0.2f), 1f).setDelay(7f)
            .setEase(LeanTweenType.easeInOutCirc).setLoopPingPong(3);
    ...
    private GameObject createSpriteDude(string name, Vector3 pos, bool hasParticles = true)
    {
        ...
        if (hasParticles)
        {
            var particles = Instantiate(prefabParticles, Vector3.zero, prefabParticles.transform.rotation);
        ...
        LeanTween.delayedCall(gameObject, 14f, () =>
            {
                for (var i = 0; i < 10; i++)
                {
                    ...
                    // Instantiate Avatar
                    var dude = createSpriteDude("dude" + i, new Vector3(-2.51208f, 7.100643f, -14.37754f),
                        false); //(GameObject)GameObject.Instantiate(prefabAvatar, Vector3.zero, prefabAvatar.transform.rotation );
                    ...
                    LeanTween.scale(dude, new Vector3(0.65f, 0.65f, 0.65f), 1f).setDelay(i * 0.2f)
                    ...
                    // Push into the wheel
                    LeanTween.moveLocalZ(dude, -2f, 0.3f).setDelay(1.2f + i * 0.4f).setEase(LeanTweenType.easeSpring)
                        .setOnComplete(
                            () => { LeanTween.rotateAround(rotator, Vector3.forward, -1080f, 12f); }
                        );

                    // Jump Up and back down
                    LeanTween.moveLocalY(dude, 1.17f, 1.2f).setDelay(5f + i * 0.2f).setLoopPingPong(1)
