...
public class SoundController : MonoBehaviour
{
    ...
    public AudioClip[]
        _audioClips; //Place audio clips (Optional: audio clips can also be played directly from gameObjects)

    public int _audioChannels = 10; //How many audio sources can play at the same time

    public float
        _masterVol =
            .5f; //Volume applied to all sound (keep at aprox 0.5 for better individual object volume control - Example: Setting object volume to 2 on events that are important)
    ...
    private float
        _currentMusicVol; //Cache for the music clips volume, makes controller able to change volume on runtime. Run UpdateMusicVolume(); after changing _musicVol
    ...
    {
        for (var j = 0; j < 2; j++) _musicChannels[j].volume = _currentMusicVol * _masterVol * _musicVol;
    }
    ...
    public void AddChannels()
    {
        ...
        if (channels.Length <= _audioChannels)
            for (var i = 0; i < _audioChannels; i++)
            {
                var chan = new GameObject();
                chan.AddComponent<AudioSource>();
                chan.name = "AudioChannel " + i;
                chan.transform.parent = transform;
        ...
        for (var j = 0; j < 2; j++)
        {
            var mchan = new GameObject();
            mchan.AddComponent<AudioSource>();
            mchan.name = "MusicChannel " + j;
            mchan.transform.parent = transform;
    ...
    public void StopAll()
    {
        ...
        for (var i = 0; i < channels.Length; i++) channels[i].Stop();
    }
}